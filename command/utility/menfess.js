...
export default {
  ...
  run: async (m, { conn, text, args, command }) => {
    ...
    if (command.includes('chat')) {
      ...
      let who = text ? text.replace(/\D/g, '') + '@s.whatsapp.net' : m.mentionedJid?.mentionedJid[0] ? m.mentionedJid[0] : '';
      if (!who) return m.reply(`tag atau ketik nomornya!`);

      let meh = await conn.onWhatsApp(who);
      if (meh.length < 1) return m.reply(`[!] Failed, @${who.split('@')[0]} bukan pengguna WhatsApp.`, null, { mentions: [who] });

      who = meh[0].jid;
      if (somematch([conn.user.jid, m.sender], who)) throw `[!] Cari nomor yang lain.`;

      if (object.length > 0) {
        let sesisender = false, sesitarget = false;
        for (let x of object) {
          let p = menfesschat[x];
          if (somematch([m.sender, who], p.a)) sesisender = true;
          if (somematch([m.sender, who], p.b)) sesitarget = true;
        }
        if (sesisender || sesitarget) return m.reply(`Anda masih berada dalam sesi *menfesschat*\n\nkeluar dari sesi dengan perintah :\n*${m.prefix}menfessstop*`);
      }
      ...
      await conn.reply(m.sender, `Anda berhasil terhubung dengan @${who.split('@')[0]}, percakapan selanjutnya akan diteruskan ke target.\n*Note :* Hanya dapat digunakan dalam private chat.\n\nGunakan perintah *${m.prefix}menfessstop* untuk keluar dari sesi.`, m, { mentions: [who] });
      await conn.reply(who, `Hi, saya Bot Menfess, Ada pengirim rahasia yang sedang terhubung denganmu, chat *_forwarded_* menandakan pesan tersebut berasal dari pengirim.\n\nKetik *.menfessstop* untuk keluar dari sesi percakapan.`);
    } else if (command.includes('stop')) {
      if (object.length == 0) throw 'Belum ada sesi *menfesschat* di Bot ini.';

      let dataku, target, a = false;
      for (let x of object) {
        let p = menfesschat[x];
      ...
      await m.reply(`Berhasil keluar dari sesi *menfesschat*.${a ? '' : `\n\nTertarik mencoba ? Ketik .menfess`}`);
      await conn.reply(target, `${a ? 'Pengirim' : 'Target'} telah menghentikan sesi percakapan dengan anda, terima kasih telah menggunakan layanan.${a ? `\n\nTertarik mencoba ? Ketik .menfesschat` : ''}`);
      delete menfesschat[dataku];
    } else if (somematch(bahasa.map(v => v + 'settimer'), command)) {
      if (!isOwner) return m.reply('*「OWNER BOT ONLY」*');

      datas.menfesschatcd = +new Date();
      let date = new Date(datas.menfesschatcd);
      m.reply(`MenfessChat reset setiap pukul ${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}`);
    } else {
      if (!text) throw `Ungkapkan perasaanmu secara anonim. Anda juga dapat mengirim media seperti audio, video, gambar, dan stiker.\n\n*Contoh Penggunaan :*\n${m.prefix + command} 6282121332749 | hai kamu\n\n*${m.prefix}menfesschat* untuk chatting dengan orang lain secara anonim\n*${m.prefix}menfessstop* untuk keluar dari sesi chatting`;
      ...
      let who = args[0] ? args[0].replace(/\D/g, '') + '@s.whatsapp.net' : m.mentionedJid?.mentionedJid[0] ? m.mentionedJid[0] : '';
      ...
      who = meh[0].jid;
      if (who.includes(conn.user.jid)) throw `[!] Tidak bisa mengirim *menfess* ke Bot`;
      ...
      let mpro, menfess = datas.menfess;
      ...
      if (m.isGroup) await m.reply(`_>> proses mengirim pesan *${mime ? mime : 'teks'}* . . ._`);

      if (mime.includes('audio')) await conn.sendMsg(who, { audio: buffer, mimetype: 'audio/mpeg', ptt: true });
      if (mime && !mime.includes('audio')) {
        if (!mime.includes('webp')) await conn.sendFile(m.sender, buffer, '', senderr, null, false, { mentions: [who] });
