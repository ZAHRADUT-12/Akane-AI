  ...
  run: async (m, { conn }) => {
    try {
      ...
      if (new Date() - user.lastkencan < cooldown) {
        m.reply(
          `Kamu sudah berkencan sebelumnya! Tunggu selama *${conn.msToDate(user.lastkencan + cooldown - new Date())}* untuk berkencan lagi!`,
        );
      }

      if (user.waifu == "") {
        return m.reply(
          "❌ Kamu belum mempunyai waifu! Ketik *.waifu* dan pilih waifumu",
        );
      }
