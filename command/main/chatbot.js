export default {
    ...
    run: async (m) => {
        ...
        if (action === "on") {
            user.chat = true;
            return m.reply("Memulai sesi chat bersama bot, balas pesan dari bot untuk melanjutkan obrolan");
        } else if (action === "off") {
            user.chat = false;
            return m.reply("Mengakhiri sesi chat bersama bot");
        } else if (action === "reset") { 
        	if (!user.messages.length > 0) return m.reply("Tidak ada history chat bot");
        	user.messages = []
