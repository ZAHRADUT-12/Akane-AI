export default {
    ...
    run: async function (m, { conn }) {
        ...
        if (user.claimCount >= (user.dailyLimit || 10)) {
            return m.reply(`Maaf, kamu telah mencapai batas klaim harian (${user.dailyLimit || 10} kali). Coba lagi besok.`);
        }
        ...
        if (lastClaim && now - lastClaim < oneDay) {
            ...
            return m.reply(`Kamu sudah mengklaim balance hari ini. Coba lagi dalam ${hours} jam ${minutes} menit ${seconds} detik.`);
        }
        ...
        return m.reply(`Selamat! Kamu telah mengklaim ${toDollar(dailyBonus)} balance dan ${user.claimCount}/${user.dailyLimit} kali hari ini.`);
    },
