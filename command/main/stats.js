...
const formatHeap = (x) => x.split('_').map(i => toUpper(i)).join(' ')
...
export default {
  ...
  run: async (m, { conn }) => {
    ...
    const personalChats = Object.values(objChats).filter(chat => !chat.id.includes('@g.us'));
    const groupChats = Object.values(objChats).filter(chat => chat.id.includes('@g.us'));
    const communityChats = Object.values(objChats).filter(chat => chat.isCommunity && chat.isCommunityAnnounce);
