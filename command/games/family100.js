export default {
    ...
    run: async (m, { conn }) => {
        ...
        if (family100_id in conn.game) {
            await m.reply("Masih ada kuis yang belum terjawab di chat ini", conn.game[family100_id].msg);
            return false;
        }

        let json = await func.fetchJson("https://raw.githubusercontent.com/BochilTeam/database/master/games/family100.json");
