export default {
  ...
  run: async function (m, { conn, text }) {
    ...
    if (isNaN(amount) || amount <= 0) {
      return m.reply(
        "Jumlah taruhan tidak valid. Harap masukkan jumlah taruhan yang benar.",
      );
    }
    ...
    const userBFormatted =
      userB.replace("@", "").replace("⁨", "").replace("⁩", "") +
      "@s.whatsapp.net";
      ...
      return new Promise((resolve) => {
        const confirmHandler = async (upsert) => {
          ...
          if (
            ...
            msg.key.participant === userBFormatted
          ) {
            resolve(true);
          }
        };
    ...
    if (confirmed) {
      if (userBData.balance < amount) {
        return m.reply(
          "User yang Anda tantang tidak memiliki saldo mencukupi untuk taruhan ini. Taruhan dibatalkan.",
        );
      }
      ...
      const newBalanceA =
        winner === "A" ? balanceA + amount : balanceA - amount;
      const newBalanceB =
        winner === "B"
          ? userBData.balance + amount
          : userBData.balance - amount;
      ...
      m.reply(
        "User B tidak mengkonfirmasi taruhan dalam waktu yang ditentukan. Taruhan dibatalkan.",
      );
    }
  },
