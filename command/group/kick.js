export default {
    ...
    run: async (m, { conn }) => {
        try {
            let who = m.quoted ? m.quoted.sender : m.mentions?.[0] ? m.mentiones[0] : m.text ? (m.text.replace(/\D/g, '') + '@s.whatsapp.net') : '';
            if (!who || who == m.sender) return m.reply('*Quote / tag* target yang ingin di kick!!');
            if (m.participants.filter(v => v.id == who).length == 0) return m.reply(`Target tidak berada dalam Grup !`);
            if (somematch([conn.user.jid, ...global.owner.map(v => v + '@s.whatsapp.net')], who)) return m.reply('Jangan gitu ama Owner');
