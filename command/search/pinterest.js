  ...
  run: async (m, { args }) => {
    try {
      if (func.isUrl(m.text)) {
        const downloadResponse = await func.fetchJson(
          API(
            "itzpire",
            `/download/pinterest?url=${encodeURIComponent(func.isUrl(m.text)[0])}`,
          ),
        );
        if (downloadResponse.status === "success" && downloadResponse.data) {
          ...
          if (image) {
            ...
            m.reply(
              "Maaf, tidak dapat menemukan gambar atau video untuk URL ini.",
            );
          }
        } else {
          m.reply(
            "Maaf, tidak dapat mengunduh gambar atau video untuk URL ini.",
          );
        }
      } else {
        const query = encodeURIComponent(m.text);
        const response = await func.fetchJson(
          API("itzpire", `/search/pinterest?query=${query}`),
        );
